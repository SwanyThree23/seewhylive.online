...
var MAX_AUDIO_SIZE = 50 * 1024 * 1024;
...
router.get('/jobs', requireAuth, async function(req, res) {
  ...
  try {
    var result = await db.query(
      ...
      'FROM music_video_jobs WHERE user_id=$1 AND tenant_id=$2 ORDER BY created_at DESC LIMIT 20',
      [userId, req.tenantId]
    );
...
router.get('/jobs/:id', requireAuth, async function(req, res) {
  ...
  try {
    var result = await db.query(
      'SELECT id, style, status, output_path, error, created_at, updated_at ' +
      'FROM music_video_jobs WHERE id=$1 AND user_id=$2 AND tenant_id=$3',
      [req.params.id, userId, req.tenantId]
    );
...
router.delete('/jobs/:id', requireAuth, async function(req, res) {
  ...
  try {
    var existing = await db.query(
      'SELECT output_path, audio_path FROM music_video_jobs WHERE id=$1 AND user_id=$2 AND tenant_id=$3',
      [req.params.id, userId, req.tenantId]
    );
    ...
    await db.query('DELETE FROM music_video_jobs WHERE id=$1 AND tenant_id=$2', [req.params.id, req.tenantId]);
