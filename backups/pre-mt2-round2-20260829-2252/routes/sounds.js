...
router.get('/', requireAuth, function(req, res) {
  var qPath = '/rest/v1/custom_sounds?creator_id=eq.' + req.user.id +
    '&tenant_id=eq.' + encodeURIComponent(req.tenantId) + '&order=created_at.desc';
...
router.post('/upload', requireAuth, soundUploadRateLimit, upload.single('audio'), function(req, res) {
  ...
  var storagePath = req.tenantId + '/' + req.user.id + '/' + Date.now() + '-' + safeName;
