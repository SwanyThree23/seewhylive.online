...
function ClipCard({ clip, onDelete, onShare }) {
  ...
  return (
    <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}}
      style={{ borderRadius:10, overflow:'hidden', border:'1px solid rgba(255,255,255,0.06)', background:'#111', cursor:'pointer' }}>
      {/* Thumbnail */}
        ...
        {highlights.length > 0 && (
          <div style={{ marginTop:32 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {highlights.map(h => (
                <div key={h.id} style={{ padding:'12px 14px', borderRadius:8, border:'1px solid rgba(255,255,255,0.06)', background:'#111', display:'flex', alignItems:'center', gap:12 }}>
