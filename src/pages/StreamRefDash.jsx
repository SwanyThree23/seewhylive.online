    ...
    <div>
      ...
      <Section title="Socket.IO Event Map">
        {[
          ...
          { event: "judge:score",      dir: "client→server", desc: "Judge submits round score via scorecard" },
          { event: "judge:ruling",     dir: "server→client", desc: "Official ruling broadcast to all viewers" },
          { event: "svs:match_update", dir: "server→client", desc: "Live bracket + standing update" },
        ].map((e) => (
    ...
    <div>
      ...
      <Section title="PM2 Processes">
        {[
          ...
          { name: "seewhylive-frontend", script: "vite preview (or nginx serve)",   port: 5173,          status: "live" },
        ].map((p) => (
    ...
    <div>
      ...
      <Section title="Core Tables">
        {[
          ...
          { table: "svs_matches",      desc: "SVS tournament matches",        cols: ["id", "tournament_id", "home_state", "away_state", "round", "status", "home_score", "away_score", "judge_id", "started_at"] },
          { table: "judge_scorecards", desc: "Per-round judge submissions",   cols: ["id", "match_id", "judge_id", "round", "home_points", "away_points", "notes", "submitted_at", "verified"] },
        ].map((t) => (
      ...
      <Section title="Database Triggers">
        {[
          ...
          { trigger: "on_judge_score_submit",   action: "Recalculate match running score, emit svs:match_update socket" },
        ].map((t) => (
      ...
      <Section title="RLS Policies">
        ...
        <Row label="svs_matches"      value="SELECT: public · UPDATE: judge or admin only" />
        <Row label="judge_scorecards" value="SELECT: judge, admin · INSERT: judge (own row)" />
      </Section>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <Section title="Injection Pattern">
        <div className="text-xs text-white/50 mb-2">Safe way to update .env from terminal:</div>
        ...
        <div className="mt-2 bg-[#C0392B]/10 border border-red-500/20 rounded p-2 text-xs text-red-200">
          ⚠ Never paste live secret keys in chat — use terminal only
        </div>
      </Section>
    </div>
  );
...
function StatusTab() {
  ...
  const runCheck = async (check) => {
    ...
    try {
      await fetch(check.url, { method: check.method === "head" ? "HEAD" : "GET", mode: "no-cors", signal: AbortSignal.timeout(5000) });
      const ms = Date.now() - start;
      setResults((r) => ({ ...r, [check.id]: { ok: true, ms } }));
    } catch (e) {
      const ms = Date.now() - start;
    ...
    <div>
      <Section title="Live Endpoint Checker">
        <div className="text-xs text-white/40 mb-4">
          Uses browser fetch — no-cors mode means "reachable" check only. Run from VPS for full HTTP status codes.
        </div>
        <button onClick={runAll} className="mb-4 w-full py-2 bg-[#800020] hover:bg-[#a00028] text-white text-sm font-mono uppercase tracking-wider rounded transition-colors">
          ▶ Check All Endpoints
        </button>
        {checks.map((check) => {
          ...
          return (
            <div key={check.id} className="flex items-center gap-3 py-3 border-b border-white/5">
              ...
              <div className="text-right">
                ...
                {!r && !isChecking && (
                  <button onClick={() => runCheck(check)} className="text-white/30 hover:text-white text-xs transition-colors">check →</button>
                )}
              </div>
            </div>
          );
...
function JudgesTab() {
  const [judgeCheckDone, setJudgeCheckDone] = useState(Array(8).fill(false));

  const judgePreMatchSteps = [
    "Confirm judge credentials in Supabase (role = 'judge', tournament_id assigned)",
    "Log in at seewhylive.online — verify ⚖️ Judge badge visible in profile",
    "Open the active SVS match from State vs State → active bracket",
    "Confirm digital scorecard loaded (Match ID visible top-right)",
    "Sync with Head Judge via Joyce AI or direct message before tip-off",
    "Verify stream overlay is receiving judge:score socket events",
    "Confirm Guardian AI is monitoring match chat for violations",
    "Acknowledge match rules with both state captains (on-stream or DM)",
  ];

  return (
    <div>
      {/* Roles */}
      <Section title="Judge Roles & Permissions">
        {[
          {
            role: "Head Judge",
            badge: "gold",
            perms: [
              "Submit and override round scores",
              "Emit official_ruling broadcast to all viewers",
              "Trigger match pause / resume via socket",
              "Escalate disputes to platform admin",
              "Access full match audit log",
              "Unlock/lock scorecard for other judges",
            ],
            icon: "⚖️",
          },
          {
            ...
            badge: "info",
            perms: [
              "Submit round scores (awaits Head Judge confirmation)",
              "Flag illegal plays via guardian:action emit",
              "View real-time opponent score (read-only)",
              "Access current match chat thread",
            ],
            icon: "📋",
          },
          {
            ...
            badge: "info",
            perms: [
              "Enter running point totals per round",
              "View match history and prior round submissions",
              "Generate end-of-match PDF scorecard",
              "Cannot override or publish rulings",
            ],
            icon: "🔢",
          },
          {
            role: "Timekeeping Judge",
            badge: "info",
            perms: [
              "Start/stop round timer (visible on stream overlay)",
              "Emit time:warning at 60s and 30s remaining",
              "Call time violations (logged as guardian_events)",
              "No scoring access",
            ],
            icon: "⏱️",
          },
        ].map((r) => (
          <div key={r.role} className="mb-4 bg-black/30 rounded-lg p-4 border border-white/10">
            <div className="flex items-center gap-3 mb-3">
              <span className="text-xl">{r.icon}</span>
              <span className="text-[#D4AF37] font-mono text-sm font-bold uppercase tracking-wider">{r.role}</span>
              <Badge type={r.badge}>{r.badge === "gold" ? "HEAD" : "STAFF"}</Badge>
            </div>
      ...
      {/* Scoring System */}
      <Section title="SVS Scoring System">
        <div className="bg-black/40 border border-[#D4AF37]/20 rounded-lg p-4 mb-4">
          <div className="text-[#D4AF37] font-mono text-xs font-bold uppercase tracking-wider mb-3">Point Calculation Rules</div>
          {[
            { rule: "Win a round (opponent blocked)",      points: "+1 Match Point" },
            { rule: "Domino out (all tiles played)",       points: "+2 Match Points" },
            { rule: "Shut-out win (opponent scores 0)",    points: "+3 Match Points" },
            { rule: "Double-six opener bonus",             points: "+0.5 (display only)" },
            { rule: "Opponent pip count at block",         points: "Added to winner's running total" },
            { rule: "Tiebreaker (equal match points)",     points: "Pip count differential decides" },
            { rule: "Forfeit",                             points: "Opponent wins 3-0 by default" },
            { rule: "Time violation (3rd offense)",        points: "−1 Match Point for violating team" },
          ].map((s) => (
            <div key={s.rule} className="flex items-start justify-between gap-4 py-2 border-b border-white/5">
              <span className="text-white/60 text-xs">{s.rule}</span>
              <span className="text-[#D4AF37] text-xs font-mono font-bold shrink-0 text-right">{s.points}</span>
            </div>
          ))}
        </div>

        <Row label="Rounds per Match" value="Best of 5 (first to 3)" badge={{ type: "info", label: "SVS format" }} />
        <Row label="Time Limit per Round" value="8 minutes" />
        <Row label="Max Tiles per Player" value="7 (standard draw)" />
        <Row label="Score Display" value="Overlay updates in real-time via svs:match_update" />
        <Row label="Scorecard Lock" value="Head Judge locks after each round confirmation" />
        <Row label="Tie Resolution" value="Highest pip count total from losing rounds" />
      </Section>

      {/* API Endpoints */}
      <Section title="Judge API Endpoints">
        <div className="text-xs text-white/50 mb-2">Base: https://seewhylive.online/api/judge</div>
        <Code>POST   /scorecard/submit</Code>
        <Code>GET    /scorecard/:matchId</Code>
        <Code>PUT    /scorecard/:scorecardId/override</Code>
        <Code>POST   /ruling/broadcast</Code>
        <Code>POST   /match/:matchId/pause</Code>
        <Code>POST   /match/:matchId/resume</Code>
        <Code>GET    /match/:matchId/audit-log</Code>
        <Code>POST   /dispute/:matchId/escalate</Code>
        <Code>GET    /scorecard/:matchId/export-pdf</Code>
        <Code>POST   /timer/:matchId/start</Code>
        <Code>POST   /timer/:matchId/stop</Code>
      </Section>

      {/* Socket Events */}
      <Section title="Judge Socket.IO Events">
        {[
          { event: "judge:score",          dir: "client→server", desc: "Submit round score payload { matchId, round, homePoints, awayPoints, notes }" },
          { event: "judge:ruling",         dir: "server→client", desc: "Official ruling text pushed to all room viewers + stream overlay" },
          { event: "judge:lock",           dir: "server→client", desc: "Head Judge locks scorecard — all line judges read-only" },
          { event: "judge:override",       dir: "client→server", desc: "Head Judge corrects a submitted score before lock" },
          { event: "judge:flag",           dir: "client→server", desc: "Line judge flags illegal play → creates guardian_events row" },
          { event: "match:pause",          dir: "client→server", desc: "Pauses round timer on stream overlay" },
          { event: "match:resume",         dir: "client→server", desc: "Resumes round timer" },
          { event: "timer:warning",        dir: "server→client", desc: "60s and 30s warnings emit to overlay and chat" },
          { event: "dispute:open",         dir: "client→server", desc: "Opens dispute thread, notifies platform admin" },
          { event: "dispute:resolved",     dir: "server→client", desc: "Admin resolves — broadcasts ruling to room" },
        ].map((e) => (
          <div key={e.event} className="py-2 border-b border-white/5">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[#D4AF37] font-mono text-xs">{e.event}</span>
              <Badge type={e.dir.startsWith("client") ? "ruby" : "info"}>{e.dir}</Badge>
            </div>
            <p className="text-white/40 text-xs mt-0.5">{e.desc}</p>
          </div>
        ))}
      ...
      {/* Overlay Controls */}
      <Section title="Stream Overlay Controls">
        <div className="bg-black/40 border border-white/10 rounded-lg p-4 mb-4">
          <div className="text-xs text-white/50 mb-3">Judges can push the following updates directly to the live stream overlay:</div>
          {[
            { control: "Score Update",      trigger: "Auto on judge:score socket confirmation",         style: "Gold scoreboard top-bar update" },
            { control: "Round Winner",      trigger: "Head Judge confirms round lock",                  style: "Full-screen flash with winning state flag" },
            { control: "Official Ruling",   trigger: "POST /ruling/broadcast",                         style: "Crimson ticker crawl for 10s" },
            { control: "Pause Banner",      trigger: "match:pause socket emit",                        style: "⏸ MATCH PAUSED overlay with reason text" },
            { control: "Timer Display",     trigger: "timer:start → counts down on overlay",           style: "Clock widget bottom-right, turns red at 30s" },
            { control: "Foul Call",         trigger: "judge:flag → guardian_events insert",            style: "Yellow ⚠ icon + \"ILLEGAL PLAY\" ticker" },
            { control: "Match Winner Card", trigger: "Final round lock + match:end event",             style: "Full celebration overlay with winning state" },
            { control: "Dispute Notice",    trigger: "dispute:open socket",                            style: "⚖️ UNDER REVIEW banner, pauses timer" },
          ].map((o) => (
            <div key={o.control} className="py-2.5 border-b border-white/5">
              <div className="flex items-center gap-2 mb-0.5">
      ...
      {/* Dispute Resolution */}
      <Section title="Dispute Resolution Flow">
        {[
          { step: 1, actor: "Line Judge",     action: "Flags play via judge:flag — timer pauses automatically" },
          { step: 2, actor: "Head Judge",     action: "Reviews replay (if VOD clip available) within 90s" },
          { step: 3, actor: "Head Judge",     action: "Issues ruling via POST /ruling/broadcast — overlay shows UNDER REVIEW" },
          { step: 4, actor: "Both Captains",  action: "Acknowledge ruling on-screen (tap Acknowledge button in SVS UI)" },
          { step: 5, actor: "Head Judge",     action: "Resolves dispute:open socket — play resumes or score adjusted" },
          { step: 6, actor: "Platform Admin", action: "Can escalate to appeal within 5 minutes if captain contests" },
          { step: 7, actor: "System",         action: "All actions logged to guardian_events + match audit log (immutable)" },
        ].map((s) => (
          <div key={s.step} className="flex gap-3 py-2 border-b border-white/5">
            <span className="text-[#800020] font-mono text-xs font-bold shrink-0 w-5">{s.step}</span>
            <div>
              <span className="text-[#D4AF37] font-mono text-xs font-bold">{s.actor}</span>
              <span className="text-white/60 text-xs ml-2">{s.action}</span>
            </div>
          </div>
        ))}
      </Section>
      ...
      {/* Pre-match checklist */}
      <Section title="Judge Pre-Match Checklist">
        <div className="text-xs text-white/40 mb-3">Tap to mark complete</div>
        {judgePreMatchSteps.map((step, i) => (
          <div key={i} onClick={() => setJudgeCheckDone((d) => d.map((v, j) => (j === i ? !v : v)))} className="cursor-pointer">
            <CheckItem done={judgeCheckDone[i]}>{step}</CheckItem>
          </div>
        ))}
        <div className="mt-2 text-xs text-[#D4AF37] font-mono">{judgeCheckDone.filter(Boolean).length} / 8 complete</div>
      </Section>

      {/* Auth & Credentials */}
      <Section title="Judge Authentication">
        <Row label="Role Grant" value="role = 'judge' in Supabase users table" badge={{ type: "info", label: "admin-set" }} />
        <Row label="Tournament Scope" value="tournament_id FK — judges scoped per event" />
        <Row label="JWT Claim" value="x-judge-role claim in AccessToken" />
        <Row label="Scorecard Access" value="Only matches where judge_id = auth.uid()" />
        <Row label="Ruling Broadcast" value="Head Judge role check on server before emit" />
      ...
      {/* Joyce AI integration */}
      <Section title="Joyce AI — Judge Assist Prompts">
        <div className="text-xs text-white/50 mb-3">Quick Joyce AI prompts for on-stream judge situations:</div>
        {[
          { prompt: "Read the official SVS illegal-play ruling for blocking all doubles.",            use: "Rule lookup" },
          { prompt: "Announce a 2-minute delay due to a tile dispute — keep it neutral and firm.",   use: "Broadcast copy" },
          { prompt: "Congratulate [State] on winning Round 3 in a 30-second hype announcement.",     use: "Round win hype" },
          { prompt: "Explain the tiebreaker pip-count rule to viewers who are confused.",             use: "Viewer education" },
          { prompt: "Generate a professional match summary for Round 1 through 3 of this SVS game.", use: "Score recap" },
        ].map((j) => (
          <div key={j.use} className="py-2.5 border-b border-white/5">
            <div className="flex items-center gap-2 mb-1">
              <Badge type="gold">{j.use}</Badge>
            </div>
            <p className="text-white/55 text-xs italic">"{j.prompt}"</p>
          </div>
        ))}
        <div className="mt-3 text-xs text-white/30">Access Joyce AI → <span className="text-[#D4AF37]">seewhylive.online/JoyceAI</span></div>
      </Section>
    </div>
  );
}
...
export default function StreamRefDash() {
  ...
  return (
    <div style={{ fontFamily: "'DM Mono', 'Courier New', monospace" }} className="min-h-screen bg-[#0a0608] text-white">
      {/* Header */}
      <div className="border-b border-[#800020]/40 bg-[#0d080a]">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-start justify-between gap-4 mb-1 flex-wrap">
            <div>
              <a href="/AdminDashboard" className="text-[#800020] text-[10px] font-mono uppercase tracking-[0.3em] mb-1 hover:text-[#D4AF37] transition-colors block">
                ← Admin Dashboard
              </a>
              ...
              <div className="text-white/30 text-[10px] font-mono mt-1">SeeWhy LIVE · SwanyThree EntTech LLC</div>
            </div>
      ...
      {/* Tab Bar */}
      <div className="border-b border-white/10 bg-[#0a0608] sticky top-0 z-10">
