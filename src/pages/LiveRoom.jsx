...
export default function LiveRoom() {
  ...
  return (
    <div className="fixed inset-0 flex flex-col overflow-hidden"
      ...
      {roomId && <StreamEventBus roomId={roomId} isHost={isHost} sessionId={roomId} onViewerUpdate={setBusViewerCount} onTipReceived={msg => setTipTotal(t => t + Math.floor(msg?.tip_amount || 0))} onMessageReceived={msg => { if (!chatOpen) setUnread(n => n + 1); if (msg?.content) { setChatMessages(prev => { const next = [...prev, msg]; return next.length > 100 ? next.slice(-100) : next; }); setHypeLevel(prev => Math.min(100, prev + 2)); } }} />}
      ...
      {roomId && <MobileStreamControls micMuted={!audioEnabled} onMicToggle={handleToggleAudio} onReact={() => {}} onQuickTip={() => !isHost && setShowTippingModal(true)} onWebSource={isHost ? () => setShowEvmux(true) : undefined} onClip={isHost ? () => setShowClipCreator(true) : undefined} onPoll={isHost ? () => setPollTick(t => t + 1) : undefined} roomId={roomId} />}
      ...
      <StreamMetricsBar startTime={elapsed > 0 ? Date.now() - elapsed * 1000 : null} memberCount={liveCount} tipTotal={tipTotal} peakViewers={peakViewers} netQuality={netQuality} netRtt={netRtt} />
