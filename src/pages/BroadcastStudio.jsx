...
export default function BroadcastStudio() {
    ...
    (async () => {
      try {
        ...
        if (!existing.length) {
          ...
          if (mounted) qc.invalidateQueries(['broadcast-members', party.id]);
        }
      } catch {}
    })();
  ...
  useEffect(() => () => {
    ...
    base44.entities.WatchPartyMember.filter({ party_id: party.id, user_id: user.id, is_active: true })
      .then(ms => ms.forEach(m => base44.entities.WatchPartyMember.update(m.id, { is_active: false, left_at: new Date().toISOString() }).catch(() => {})))
      .catch(() => {});
  }, [party?.id, user?.id]);
...
Respond with JSON only: {"genre": "one of: Lo-Fi|Trap|Gospel|Afrobeats|R&B|Chill|Hype|Jazz|EDM|Soul", "mood": "one sentence mood description", "energy": "low|medium|high"}`
  ...
  const promoteCoHost = async (member) => {
    try {
      ...
      qc.invalidateQueries(['broadcast-members', partyId]);
    } catch { toast.error('Failed to promote member. Please try again.'); }
  };
  ...
  const promoteSpeaker = async (member) => {
    try {
      ...
      qc.invalidateQueries(['broadcast-members', partyId]);
    } catch { toast.error('Failed to add member to panel. Please try again.'); }
  };
  ...
  const demoteToAudience = async (member) => {
    try {
      ...
      qc.invalidateQueries(['broadcast-members', partyId]);
    } catch { toast.error('Failed to update member role. Please try again.'); }
  };
  ...
  const kickMember = async (member) => {
    try {
      ...
      qc.invalidateQueries(['broadcast-members', partyId]);
    } catch { toast.error('Failed to remove member. Please try again.'); }
  };
  ...
  const sendRaiseHand = async () => {
    ...
    } catch { toast.error('Failed to raise hand. Please try again.'); }
  };
  ...
  // ── Entry gate ───────────────────────────────────────────────────────────
  const studioRole = party?.host_id === user?.id ? 'host'
    : members.some(m => m.user_id === user?.id && m.role === 'co-host') ? 'co-host'
    : 'speaker';
