  ...
  return (
    <div style={{ background: C.bg, minHeight: 300 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, padding: "12px 0 8px" }}>
        {[
          ...
          <div key={s.label} style={{ background: C.card, borderRadius: 8, padding: "10px", textAlign: "center", border: "1px solid #2a2a2a" }}>
            ...
            <div style={{ fontFamily: C.fMon, fontSize: 8, color: C.dim, letterSpacing: 1 }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #2a2a2a", marginBottom: 12 }}>
        {["items", "orders"].map(v => (
          <button key={v} onClick={() => setView(v)} style={{ flex: 1, padding: "8px", background: "none", border: "none", borderBottom: "2px solid " + (view === v ? C.gold : "transparent"), cursor: "pointer", fontFamily: C.fMon, fontSize: 9, letterSpacing: 1, color: view === v ? C.gold : C.gray }}>{v.toUpperCase()}</button>
        ))}
      </div>
            ...
            <div style={{ background: C.card, border: "1px solid " + C.burgundy, borderRadius: 10, padding: 14, marginBottom: 10 }}>
              ...
              {[{ key: "name", label: "Name", ph: "Item name" }, { key: "price_usd", label: "Price (USD)", ph: "9.99" }, { key: "description", label: "Description", ph: "Optional" }].map(f => (
                <div key={f.key} style={{ marginBottom: 8 }}>
                  <div style={{ fontFamily: C.fMon, fontSize: 8, color: C.dim, letterSpacing: 1, marginBottom: 4 }}>{f.label}</div>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                <button onClick={() => setShowAdd(false)} style={{ flex: 1, padding: "8px", borderRadius: 6, border: "1px solid #333", background: C.surface, color: C.gray, cursor: "pointer", fontFamily: C.fMon, fontSize: 9 }}>CANCEL</button>
                <button onClick={() => addMutation.mutate()} style={{ flex: 1, padding: "8px", borderRadius: 6, border: "none", background: C.burgundy, color: C.gold, cursor: "pointer", fontFamily: C.fMon, fontSize: 9 }}>CREATE</button>
              </div>
            </div>
          )}
          ...
          {items.map(item => (
            <div key={item.id} style={{ background: C.card, border: "1px solid #2a2a2a", borderRadius: 8, padding: "10px 12px", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontFamily: C.fMon, fontSize: 9, color: C.gold }}>${item.price_usd}</div>
              </div>
            ...
            <div key={order.id} style={{ background: C.card, border: "1px solid #2a2a2a", borderRadius: 8, padding: "10px 12px", marginBottom: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
                <div>
                  ...
                  <div style={{ fontFamily: C.fMon, fontSize: 8, color: C.dim }}>{order.buyer_name} {order.size ? "· " + order.size : ""}</div>
                </div>
                <div style={{ fontFamily: C.fBeb, fontSize: 16, color: C.gold }}>${order.total_usd}</div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ padding: "2px 8px", borderRadius: 10, fontFamily: C.fMon, fontSize: 8, background: (STATUS_COLORS[order.status] || C.gray) + "22", border: "1px solid " + (STATUS_COLORS[order.status] || C.gray) + "66", color: STATUS_COLORS[order.status] || C.gray }}>{order.status}</span>
                ...
                <select value={order.status} onChange={e => updateOrderMutation.mutate({ id: order.id, status: e.target.value })}
                  style={{ marginLeft: "auto", background: "#111", border: "1px solid #333", color: C.white, borderRadius: 4, padding: "2px 6px", fontFamily: C.fMon, fontSize: 8, cursor: "pointer" }}>
