...
// Rules: no optchain · no nullish · no localStore · Math.floor() for money · inline styles only
...
function StreamerCard({ streamer, onFollow, onSuper }) {
  var s = streamer || { name:"SwanyThree23", handle:"swanythree23", viewers:312, category:"Domino x Tech", isLive:true, tier:"gold", gems:4200 };
  var tierColor = { gold:"#C9A84C", platinum:"#E5E4E2", silver:"#C0C0C0", bronze:"#CD7F32" };
  var tc = tierColor[s.tier] || "#888";
  return React.createElement("div", { style:{ background:"#111", borderRadius:14, overflow:"hidden", border:"1px solid #1a1a1a", fontFamily:"'Barlow Condensed',sans-serif" } },
    React.createElement("div", { style:{ position:"relative", aspectRatio:"16/9", background:"#050505", display:"flex", alignItems:"center", justifyContent:"center" } },
      React.createElement("div", { style:{ fontFamily:"'Bebas Neue',sans-serif", fontSize:48, color:"#1a1a1a", letterSpacing:4 } }, s.name.charAt(0)),
      s.isLive && React.createElement("div", { style:{ position:"absolute", top:8, left:8, background:"#ff2244", borderRadius:4, padding:"2px 8px", fontSize:11, color:"#fff" } }, "LIVE"),
      s.isLive && React.createElement("div", { style:{ position:"absolute", top:8, right:8, background:"rgba(0,0,0,0.7)", borderRadius:4, padding:"2px 8px", fontSize:11, color:"#aaa" } }, "👁 "+s.viewers)
    ),
    React.createElement("div", { style:{ padding:"12px 14px" } },
      React.createElement("div", { style:{ display:"flex", alignItems:"center", gap:10, marginBottom:8 } },
        React.createElement("div", { style:{ width:38, height:38, borderRadius:"50%", background:tc+"22", border:"2px solid "+tc, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, color:tc } }, s.name.charAt(0)),
        React.createElement("div", { style:{ flex:1 } },
          React.createElement("div", { style:{ fontWeight:700, fontSize:15, color:"#fff" } }, s.name),
          React.createElement("div", { style:{ fontSize:12, color:"#555" } }, "@"+(s.handle || s.name.toLowerCase()))
        ),
        React.createElement("div", { style:{ fontSize:12, color:"#C8F030" } }, "💎 "+s.gems.toLocaleString())
      ),
      React.createElement("div", { style:{ fontSize:12, color:"#666", marginBottom:12 } }, s.category),
      React.createElement("div", { style:{ display:"flex", gap:8 } },
        React.createElement("button", { onClick:onFollow, style:{ flex:1, padding:"8px 0", background:"#800020", border:"none", borderRadius:8, color:"#fff", fontFamily:"'Bebas Neue',sans-serif", fontSize:14, letterSpacing:1, cursor:"pointer" } }, "FOLLOW"),
        React.createElement("button", { onClick:onSuper, style:{ width:38, height:36, background:"#C9A84C22", border:"1px solid #C9A84C44", borderRadius:8, color:"#C9A84C", fontSize:16, cursor:"pointer" } }, "💎")
      )
    )
  );
}

function OctagonalGrid({ streams, onSelect }) {
  var items = streams || [
    { id:1, name:"SwanyThree23", viewers:312, isLive:true,  category:"Domino x Tech", gems:4200 },
    { id:2, name:"ClassicLive",  viewers:188, isLive:true,  category:"Washington Classic", gems:1850 },
    { id:3, name:"AIverse_Fan",  viewers:97,  isLive:true,  category:"Tech x Culture", gems:990 },
    { id:4, name:"DominoKing",   viewers:445, isLive:true,  category:"Domino", gems:3100 },
    { id:5, name:"TechMunity1",  viewers:55,  isLive:false, category:"STEAM Education", gems:420 },
    { id:6, name:"NightOwl88",   viewers:128, isLive:true,  category:"Late Night", gems:780 },
    { id:7, name:"DCDomino",     viewers:203, isLive:true,  category:"DC Classic", gems:1240 },
    { id:8, name:"SwanyFam",     viewers:0,   isLive:false, category:"SwanyFam", gems:220 },
  ];
  return React.createElement("div", { style:{ padding:"16px", fontFamily:"'Barlow Condensed',sans-serif" } },
    React.createElement("div", { style:{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 } },
      React.createElement("div", { style:{ fontFamily:"'Bebas Neue',sans-serif", fontSize:22, color:"#C9A84C", letterSpacing:2 } }, "LIVE NOW"),
      React.createElement("div", { style:{ fontSize:12, color:"#555" } }, items.filter(function(s){ return s.isLive; }).length+" streams live")
    ),
    React.createElement("div", { style:{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 } },
      items.map(function(s) {
        return React.createElement("div", { key:s.id, onClick:function(){ if(onSelect) onSelect(s); }, style:{ background:"#111", borderRadius:12, overflow:"hidden", border:"1px solid #1a1a1a", cursor:"pointer", opacity:s.isLive?1:0.5 } },
          React.createElement("div", { style:{ aspectRatio:"16/9", background:"#050505", display:"flex", alignItems:"center", justifyContent:"center", position:"relative" } },
            React.createElement("div", { style:{ fontFamily:"'Bebas Neue',sans-serif", fontSize:28, color:"#1a1a1a" } }, s.name.charAt(0)),
            s.isLive && React.createElement("div", { style:{ position:"absolute", top:6, left:6, background:"#ff2244", borderRadius:4, padding:"1px 6px", fontSize:10, color:"#fff" } }, "LIVE"),
            s.isLive && React.createElement("div", { style:{ position:"absolute", top:6, right:6, background:"rgba(0,0,0,0.7)", borderRadius:4, padding:"1px 6px", fontSize:10, color:"#aaa" } }, "👁 "+s.viewers)
          ),
          React.createElement("div", { style:{ padding:"8px 10px" } },
            React.createElement("div", { style:{ fontSize:13, fontWeight:700, color:"#fff", marginBottom:2 } }, s.name),
            React.createElement("div", { style:{ fontSize:11, color:"#555", marginBottom:4 } }, s.category),
            React.createElement("div", { style:{ fontSize:11, color:"#C8F030" } }, "💎 "+s.gems.toLocaleString())
          )
        );
      })
    )
...
function FeaturedBanner({ onWatch }) {
  var events = [
    { id:1, title:"WASHINGTON CLASSIC 2026", subtitle:"16-Team Domino Championship - Live from Jamar's Sports Bar", cta:"WATCH NOW", color:"#800020", accent:"#C9A84C" },
    { id:2, title:"AURA AI SURGE", subtitle:"SwanyThree23 is on SURGE state - 4200+ gems tonight", cta:"JOIN LIVE", color:"#3d0050", accent:"#C8F030" },
    { id:3, title:"TECHMUNITY PRESENTS", subtitle:"AIverse Podcast x SeeWhy LIVE crossover stream", cta:"SET REMINDER", color:"#001a3d", accent:"#C9A84C" },
  ];
  var [idx, setIdx] = React.useState(0);
  React.useEffect(function() {
    var t = setInterval(function() { setIdx(function(p) { return (p+1) % events.length; }); }, 5000);
    return function() { clearInterval(t); };
  }, []);
  var ev = events[idx];
  return React.createElement("div", { style:{ position:"relative", overflow:"hidden", borderRadius:14, marginBottom:16 } },
    React.createElement("div", { style:{ background:"linear-gradient(135deg,"+ev.color+",#0a0a0a)", padding:"24px 20px", minHeight:140 } },
      React.createElement("div", { style:{ fontFamily:"'Bebas Neue',sans-serif", fontSize:11, letterSpacing:4, color:ev.accent, marginBottom:6 } }, "FEATURED"),
      React.createElement("div", { style:{ fontFamily:"'Bebas Neue',sans-serif", fontSize:26, color:"#fff", letterSpacing:2, lineHeight:1.1, marginBottom:8 } }, ev.title),
      React.createElement("div", { style:{ fontSize:13, color:"rgba(255,255,255,0.6)", marginBottom:16 } }, ev.subtitle),
      React.createElement("button", { onClick:function(){ if(onWatch) onWatch(ev); }, style:{ padding:"8px 22px", background:ev.accent, color:"#000", border:"none", borderRadius:8, fontFamily:"'Bebas Neue',sans-serif", fontSize:16, letterSpacing:2, cursor:"pointer" } }, ev.cta)
    ),
    React.createElement("div", { style:{ position:"absolute", bottom:12, right:12, display:"flex", gap:5 } },
      events.map(function(_, i) {
        return React.createElement("div", { key:i, onClick:function(){ setIdx(i); }, style:{ width:i===idx?20:6, height:6, borderRadius:3, background:i===idx?ev.accent:"#333", cursor:"pointer" } });
      })
    )
  );
}

function DiscoverFeed({ onSelect }) {
  var categories = ["All","Domino","Tech","Hip-Hop","Education","Classic"];
  var [cat, setCat] = React.useState("All");
  var creators = [
    { id:1, name:"SwanyThree23", category:"Domino Tech", followers:"12.4K", isLive:true,  gems:4200, desc:"Techmunity founder. Washington Classic host." },
    { id:2, name:"Joyce Moore",  category:"Memoir Talk", followers:"8.1K",  isLive:false, gems:1200, desc:"Memoirs of a Shy Girl. Community storytelling." },
    { id:3, name:"AIverse Pod",  category:"Tech Culture",followers:"5.6K",  isLive:true,  gems:980,  desc:"AI x Hip-Hop culture podcast. Live sessions." },
    { id:4, name:"Isaac Hayes3", category:"Domino Tech", followers:"19.2K", isLive:false, gems:3400, desc:"Domino Entertainment CEO. Business x Culture." },
    { id:5, name:"DominoKing",   category:"Domino",      followers:"6.8K",  isLive:true,  gems:2100, desc:"DMV domino champion. PK battle specialist." },
    { id:6, name:"ClassicLive",  category:"Classic",     followers:"3.2K",  isLive:true,  gems:750,  desc:"Washington Classic official stream partner." },
  ];
  var filtered = cat === "All" ? creators : creators.filter(function(c){ return c.category.indexOf(cat) > -1; });
  return React.createElement("div", { style:{ fontFamily:"'Barlow Condensed',sans-serif", padding:"0 16px" } },
    React.createElement("div", { style:{ display:"flex", gap:8, overflowX:"auto", marginBottom:14, paddingBottom:4 } },
      categories.map(function(c) {
        return React.createElement("button", { key:c, onClick:function(){ setCat(c); }, style:{ padding:"5px 14px", background:cat===c?"#800020":"#111", border:cat===c?"none":"1px solid #222", borderRadius:20, color:cat===c?"#fff":"#666", fontSize:13, cursor:"pointer", whiteSpace:"nowrap" } }, c);
      })
    ),
    React.createElement("div", { style:{ display:"flex", flexDirection:"column", gap:10 } },
      filtered.map(function(cr) {
        return React.createElement("div", { key:cr.id, onClick:function(){ if(onSelect) onSelect(cr); }, style:{ background:"#111", borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", gap:12, border:"1px solid #1a1a1a", cursor:"pointer" } },
          React.createElement("div", { style:{ width:44, height:44, borderRadius:"50%", background:"#1a1a1a", border:"2px solid "+(cr.isLive?"#800020":"#222"), display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, color:cr.isLive?"#C9A84C":"#444", flexShrink:0 } }, cr.name.charAt(0)),
          React.createElement("div", { style:{ flex:1, minWidth:0 } },
            React.createElement("div", { style:{ display:"flex", alignItems:"center", gap:8, marginBottom:2 } },
              React.createElement("span", { style:{ fontWeight:700, fontSize:15, color:"#fff" } }, cr.name),
              cr.isLive && React.createElement("span", { style:{ background:"#ff2244", borderRadius:4, padding:"1px 6px", fontSize:10, color:"#fff" } }, "LIVE")
            ),
            React.createElement("div", { style:{ fontSize:12, color:"#555", marginBottom:2 } }, cr.category+" - "+cr.followers+" followers"),
            React.createElement("div", { style:{ fontSize:12, color:"#666" } }, cr.desc)
          ),
          React.createElement("div", { style:{ fontSize:12, color:"#C8F030", flexShrink:0 } }, "💎 "+cr.gems.toLocaleString())
        );
      })
    )
  );
