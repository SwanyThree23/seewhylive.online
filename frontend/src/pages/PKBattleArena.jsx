...
// Aligned to real pk_battles schema: defender_id, challenger_points/defender_points,
// single room_id, statuses: pending → accepted → active → ended.
//
// Props:
...
var PKBattleArena = function (props) {
  ...
  useEffect(function () {
    ...
    // Join the battle's Socket.IO room so we receive real-time ticks/scores.
    socket.emit('battle:watch', { battleId: battleId });

    var handleAccept = function (payload) {
      if ((payload.id || payload.battleId) !== battleId) return;
      // Challenger joins the room after defender accepts.
      socket.emit('battle:watch', { battleId: battleId });
    };
  ...
  // name-plate still renders when the guest hasn't joined yet.
