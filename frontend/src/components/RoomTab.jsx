...
export default function RoomTab({ socket, guests, chat, isLive, setIsLive, userId, username, role, roomId, branding, addToast, overlayConfig, viewerCount }) {
  ...
  useEffect(function() {
    ...
    socket.on('panel:camera_command',     onPanelCameraCommand);
    socket.on('panel:background_command', onPanelBackgroundCommand);
    return function() {
      ...
      socket.off('panel:camera_command',     onPanelCameraCommand);
      socket.off('panel:background_command', onPanelBackgroundCommand);
  ...
  async function onPanelCameraCommand(payload) {
    if (!payload || payload.userId !== userId) return;
    try {
      var stream = await navigator.mediaDevices.getUserMedia({
        video: payload.deviceId ? { deviceId: { exact: payload.deviceId } } : true,
        audio: false
      });
  ...
  async function handleMediaApply(config) {
    setMediaConfig(config);
    setIsMuted(false);
    setIsCamOff(false);
    if (config && config.camId) {
      try {
        var stream = await navigator.mediaDevices.getUserMedia({
          video: { deviceId: { exact: config.camId } },
          audio: false
        });
        var track = stream.getVideoTracks()[0];
